...
export type BikeLookupResult = AllUsersBike & {
    type: string;
    ownerSummary: string;
};
...
function parseBikeObject(raw: ObjectResponse): AllUsersBike | null {
...
export function useAllUsersInfoLookup() {
...
/** 查詢目前使用者擁有的 BikeNFT 清單 */
export function useAllUsersOwnedBikes() {
    const client = useIotaClient();
    const account = useCurrentAccount();

    const [bikes, setBikes] = useState<AllUsersBike[]>([]);
    const [isPending, setIsPending] = useState(false);
    const [error, setError] = useState<string | null>(null);
...
export function useAllUsersBikeLookup() {
    const client = useIotaClient();
...
/** 查詢指向目前使用者地址的待接收 SaleOrder */
export function useAllUsersPendingOrders() {
    const client = useIotaClient();
    const account = useCurrentAccount();

    const [orders, setOrders] = useState<AllUsersOrder[]>([]);
    const [isPending, setIsPending] = useState(false);
    const [error, setError] = useState<string | null>(null);
...
/** 使用者接收訂單 */
export function useAllUsersReceiveOrder() {
